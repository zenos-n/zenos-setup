...
class RunnerSafetyTests(unittest.TestCase):
    def test_hardware_lowering_accepts_generator_header(self):
        source = '''# Do not modify this file! It was generated by nixos-generate-config.
...
'''
        lowered = runner._lower_hardware_module(source)
        self.assertIn('legacy = {', lowered)
        self.assertIn('"virtio_pci"', lowered)
        self.assertIn('nixpkgs.hostPlatform = "x86_64-linux";', lowered)

    def test_oobe_state_is_read_without_nix_eval(self):
        with tempfile.TemporaryDirectory() as directory:
            host = Path(directory, "hosts/test")
            ...
            for source, expected in (
                ...
                with mock.patch.object(runner.subprocess, "run") as run:
                    self.assertIs(runner._read_oobe_enabled(directory, "test"), expected)
    ...
    def test_short_install_persists_oobe_in_zcfg_and_uses_only_disko(self):
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            with open(os.path.join(host_dir, "system.zcfg"), encoding="utf-8") as file:
                self.assertIn("oobeMode = true;", file.read())
    ...
    def test_rebuild_failure_retains_marker_and_temporary_host(self):
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            with open(os.path.join(temporary_dir, "system.zcfg"), encoding="utf-8") as file:
                self.assertIn("system.oobeMode = true;", file.read())
            self.assertFalse(
